...
func CreateTestClientWithMock(cfg *model.Cfg) (*Client, *MockDBService) {
	...
	// Create a db.Service with the mock collections
	// We can't set private fields, so we'll create the client and manually
	// inject mock behavior in tests
	dbService := &db.Service{}

	client, _ := New(ctx, dbService, cfg, tracer, log)
