...
type CredentialIssuerMetadataParameters struct {
	...
	//BatchCredentialEndpoint: OPTIONAL. URL of the Credential Issuer's Batch Credential Endpoint, as defined in Section 8. This URL MUST use the https scheme and MAY contain port, path, and query parameter components. If omitted, the Credential Issuer does not support the Batch Credential Endpoint.
	...
	CredentialResponseEncryption *MetadataCredentialResponseEncryption `json:"credential_response_encryption,omitempty" validate:"omitempty"`

	// CredentialIdentifiersSupported: OPTIONAL. Boolean value specifying whether the Credential Issuer supports returning credential_identifiers parameter in the authorization_details Token Response parameter, with true indicating support. If omitted, the default value is false.
	...
	CredentialConfigurationsSupported map[string]CredentialConfigurationsSupported `json:"credential_configurations_supported" validate:"required"`
}
...
type MetadataCredentialResponseEncryption struct {
	...
	AlgValuesSupported []string `json:"alg_values_supported" validate:"required"`
	...
	EncValuesSupported []string `json:"enc_values_supported" validate:"required"`
...
type MetadataDisplay struct {
	...
	Locale string `json:"locale,omitempty" validate:"bcp47_language_tag"`
	...
	Logo MetadataLogo `json:"logo,omitempty,omitempty"`
}
...
type CredentialConfigurationsSupported struct {
	...
	CredentialDefinition CredentialDefinition `json:"credential_definition" validate:"required"`
	...
	ProofsTypesSupported map[string]ProofsTypesSupported `json:"proofs_types_supported,omitempty"`
...
type ProofsTypesSupported struct {
	...
	ProofSigningAlgValuesSupported []string `json:"proof_signing_alg_values_supported" validate:"required"`
}
...
type CredentialDefinition struct {
	...
	CredentialSubject map[string]CredentialSubject `json:"credentialSubject" validate:"required"`
}
